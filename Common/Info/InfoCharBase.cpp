...
DWORD CInfoCharBase::GetDataSizeNo(int nNo)
{
	...
	case 79:	dwRet = sizeof(m_sizeSearchDistance);	break;	// 策敵範囲
	// NPC発生
	case 80:	dwRet = sizeof(m_dwPutCycle);	break;	// 発生周期
	case 81:	dwRet = sizeof(m_nPutMoveType);	break;	// 発生させる移動種別
	case 82:	dwRet = sizeof(m_nMaxPutCount);	break;	// 同時発生数
	case 83:	dwRet = sizeof(m_nPutAverage);	break;	// 発生確率
	case 84:	dwRet = sizeof(m_ptPutArea);	break;	// 発生範囲(半径)
	}
...
PBYTE CInfoCharBase::GetWriteData(int nNo, PDWORD pdwSize)
{
	...
	*pdwSize	= dwSize;
	...
	case 79:	pSrc = (PBYTE)&m_sizeSearchDistance;	break;	// 策敵範囲
	// NPC発生
	case 80:	pSrc = (PBYTE)&m_dwPutCycle;	break;	// 発生周期
	case 81:	pSrc = (PBYTE)&m_nPutMoveType;	break;	// 発生させる移動種別
	case 82:	pSrc = (PBYTE)&m_nMaxPutCount;	break;	// 同時発生数
	case 83:	pSrc = (PBYTE)&m_nPutAverage;	break;	// 発生確率
	case 84:	pSrc = (PBYTE)&m_ptPutArea;	break;	// 発生範囲(半径)
	}
	...
	int nNo)	// [in] 要素番号
{
	...
	case 31:
		m_strCharName = (LPCTSTR)LegacyAnsiToTString((LPCSTR)pSrc);
		dwSize = (DWORD)(strlen((LPCSTR)pSrc) + 1);
		break;
	case 32:
		m_strSpeak = (LPCTSTR)LegacyAnsiToTString((LPCSTR)pSrc);
		dwSize = (DWORD)(strlen((LPCSTR)pSrc) + 1);
		break;
	case 33:
		m_strTalk = (LPCTSTR)LegacyAnsiToTString((LPCSTR)pSrc);
		dwSize = (DWORD)(strlen((LPCSTR)pSrc) + 1);
		break;
	case 34:
		pTmp	= pSrc;
	...
	case 79:	pDst = (PBYTE)&m_sizeSearchDistance;	dwSize = sizeof(m_sizeSearchDistance);	break;	// 策敵範囲
	// NPC発生
	case 80:	pDst = (PBYTE)&m_dwPutCycle;	dwSize = sizeof(m_dwPutCycle);	break;	// 発生周期
	case 81:	pDst = (PBYTE)&m_nPutMoveType;	dwSize = sizeof(m_nPutMoveType);	break;	// 発生させる移動種別
	case 82:	pDst = (PBYTE)&m_nMaxPutCount;	dwSize = sizeof(m_nMaxPutCount);	break;	// 同時発生数
	case 83:	pDst = (PBYTE)&m_nPutAverage;	dwSize = sizeof(m_nPutAverage);	break;	// 発生確率
	case 84:	pDst = (PBYTE)&m_ptPutArea;	dwSize = sizeof(m_ptPutArea);	break;	// 発生範囲(半径)
	}
...
PBYTE CInfoCharBase::GetSendData(void)
{
	...
	dwTmp = 0;
	CopyMemoryRenew(pDataTmp, &dwTmp, sizeof(dwTmp), pDataTmp);
	CopyMemoryRenew(pDataTmp, &m_sizeSearchDistance, sizeof(m_sizeSearchDistance), pDataTmp);	// 策敵範囲
...
void CInfoCharBase::Copy(CInfoCharBase *pSrc)
{
	...
	m_sizeSearchDistance = pSrc->m_sizeSearchDistance;	// 策敵範囲
