/// @file PacketCHAR_MOVE_STOP.h
/// @brief コマンド(キャラ系:移動停止通知) 定義ファイル
/// @date 2026/02/28
...
typedef class CPacketCHAR_MOVE_STOP : public CPacketBase
{
public:
	CPacketCHAR_MOVE_STOP()
	{
		m_bUpdate = FALSE;
		m_dwMapID = 0;
		m_dwCharID = 0;
	...
	void Make(DWORD dwMapID, DWORD dwCharID, int nDirection, int x, int y, BOOL bUpdate, int nSpeedLevel, DWORD dwTimeStamp)
	{
		...
		pPacketBase->byCmdSub = SBOCOMMANDID_SUB_CHAR_MOVE_STOP;
...
public:
	...
	int m_nSpeedLevel;
	DWORD m_dwTimeStamp;
} CPacketCHAR_MOVE_STOP, *PCPacketCHAR_MOVE_STOP;
