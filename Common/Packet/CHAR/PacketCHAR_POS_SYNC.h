/// @file PacketCHAR_POS_SYNC.h
/// @brief コマンド(キャラ系:座標同期通知) 定義ファイル
/// @date 2026/02/28
...
typedef class CPacketCHAR_POS_SYNC : public CPacketBase
{
public:
	CPacketCHAR_POS_SYNC()
	{
		m_bUpdate = FALSE;
		m_dwMapID = 0;
		m_dwCharID = 0;
	...
	void Make(DWORD dwMapID, DWORD dwCharID, int nDirection, int x, int y, BOOL bUpdate, int nSpeedLevel, DWORD dwTimeStamp)
	{
		...
		pPacketBase->byCmdSub = SBOCOMMANDID_SUB_CHAR_POS_SYNC;
...
public:
	...
	int m_nSpeedLevel;
	DWORD m_dwTimeStamp;
} CPacketCHAR_POS_SYNC, *PCPacketCHAR_POS_SYNC;
