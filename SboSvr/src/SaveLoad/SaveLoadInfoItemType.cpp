...
void CSaveLoadInfoItemType::SaveToNormalTable(void)
{
	...
	if (nRet != SQLITE_OK) {
		SboPlatform::WriteDebugLine("SaveLoadInfoItemType::SaveToNormalTable: prepare failed\n");
		return;
	}
...
BOOL CSaveLoadInfoItemType::MigrateFromBlob(PCLibInfoBase pDst)
{
	...
	if (s_pDb != NULL) {
		...
		SboPlatform::WriteDebugLine("SaveLoadInfoItemType: BLOB → 正規化テーブルへマイグレーション完了\n");
	}
...
void CSaveLoadInfoItemType::Load(PCLibInfoBase pDst)
{
	...
	if (LoadFromNormalTable(pDst)) {
		SboPlatform::WriteDebugLine("SaveLoadInfoItemType: 正規化テーブルから読み込み成功\n");
		return;
	}

	// 2. 行がなければ BLOB / .dat からマイグレーション
	SboPlatform::WriteDebugLine("SaveLoadInfoItemType: 正規化テーブルが空 → BLOB/.dat からマイグレーション\n");
