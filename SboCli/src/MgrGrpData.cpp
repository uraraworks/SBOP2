...
{
	// SboGrpData.rc のリソース定義に基づくマッピングは Common/GrpLayout.cpp に一本化した
	// （SboSvr 側からも同じ解決が必要になったため）。
	const char *pszFileName = GrpLayout_GetResFileName(pszResName);
	if (pszFileName == NULL) {
		return FALSE;
	}
	_tcscpy_s(pszDst, nDstCount, AnsiToTString(pszFileName));
	return TRUE;
}
