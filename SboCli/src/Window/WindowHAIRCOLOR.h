/// @file WindowHAIRCOLOR.h
/// @brief 髪色選択ウィンドウクラス 定義ファイル
...
{
public:
			CWindowHAIRCOLOR();	// コンストラクタ
	virtual ~CWindowHAIRCOLOR();	// デストラクタ

	void	Create(CMgrData *pMgrData);	// 作成
	void	Draw(CImg32 *pDst);	// 描画
	BOOL	TimerProc(void);	// 時間処理
	void	SetType(WORD wHairColor);	// 髪色を設定
	WORD	GetType(void);	// 髪色を取得
