...
- 残課題: IP 列は対応済み（2026-09-13）: WebSocketBridge が accept 時の相手 IP（直接の相手が loopback の時だけ X-Forwarded-For の右端）をローカルポート→実 IP の対応表 ProxyIpRegistry に登録し、ゲーム側ログインで GetPeerPort から引いて IP 拒否判定と m_dwIP に使う。本番は IIS が localhost へ逆プロキシし ARR の X-Forwarded-For 付与が有効であることの確認が必要。IPv6 は未対応。キック・ブロードキャスト・サーバー保存・監査ログ永続化は未着手。狭いペインで一覧の列が折り返される。
