...
- 確認済み（2026-09-13, iPhone 実機）: ノッチ側（safe-area）の欠けなし、ホーム画面から起動（PWA スタンドアロン）で問題なし。未確認のまま: iPad 等＋物理キーボード接続（ソフトキーボードで入力できるため問題が報告されてから調査でよい）。
- 次: 4 章 Step 2（「座る」「向き固定」ボタン、ボタン表記の役割化、パッド表示の ON/OFF 設定）。

> 2026-09-13 追記（2）。

- 4 章 Step 2 完了: パッドのボタンを 3 列×2 段に（防御/攻撃・戦闘・メニュー・座る=R・向き固定=Shift）。Z/X の表記は「戦闘モード中かつウィンドウ無し＝防御/攻撃、それ以外（街など普段・ウィンドウ中）＝戻る/決定」（C++ から EM_JS で `sbop2OnWindowStateChange`＝MgrWindow::SetActive、`sbop2OnBattleModeChange`＝StateProcMAP の TimerProc で戦闘系 MoveState の変化を通知）。「向き固定」はトグル式（PC のマウスでは同時押しできないため。ウィンドウ表示・ページ非表示・パッド OFF で自動解除）。パッド表示は localStorage `sbop2.virtualPad`（auto/on/off）とオプション「入力設定」2 行目（自動/表示/非表示）で切替（ブラウザ版の ini は再読み込みで消えるため localStorage を正とする）。
- 不具合修正: オプション 2 行目が `Format(%s)` の方言差でブラウザ版だけ空表示だった → 文字列連結に。パッド表示中は物理キーボードが効かなかった（focusGameInput が mobile-pad で早期 return）→ キー入力起点の時だけ IME 入力欄にフォーカスし、最初の 1 打はパッドと同じ注入経路で補う（タップではフォーカスしない、パッド表示中は inputmode="none"）。
- 関連するサーバーの回帰修正: 座ったまま向きを変えると立ち上がっていた（Dead Reckoning 移行 a5a1e58 の回帰。サーバーが位置不変の MOVE_START/DIR_CHANGE で MOVE、非移動中の MOVE_STOP で STAND に書き換えていた）→ MoveStateDecision で位置不変・非移動中は状態を変えないよう修正。
- 確認済み（2026-09-13, iPhone 実機）: 文字入力（パッド表示中にソフトキーボードが勝手に出ない・チャット入力できる）、ノッチ側の欠けなし、PWA スタンドアロンで問題なし。未確認のまま: iPad 等＋物理キーボード接続（ソフトキーボードで入力できるため問題が報告されてから調査でよい）。
- 残課題: 4 章 Step 1（PC キー操作の棚卸し: 死にキー 9 個・コマンドメニューの表記と実装の食い違い）、5-6（iOS Chrome の自動入力バー）、ブラウザ版の他のオプション設定も再読み込みで消える可能性（未実測）。
