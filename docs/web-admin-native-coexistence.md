...
### 埋め込み Web クライアントからの直接操作

管理画面内に常設した Web クライアントでクリックした対象は、サーバーを経由せず親画面へ `postMessage` で通知する。これにより、右側の編集パネルを即時に切り替えられる。

| 発生元 | 通知 | 管理画面側の動作 | サーバー通信 |
| ------ | ---- | ---------------- | ------------ |
| Web クライアントのキャラクリック | `sbop2_admin_char_pick` | `character-overview` を開き、対象 charId の詳細を取得 | 詳細取得 `GET /api/characters/{charId}` のみ |

実装ポイント:

- `SboCli/src/StateProc/StateProcMAP.cpp`
  - Emscripten ビルド時だけ `SBOP2_PostAdminCharPick()` を有効化。
  - キャラの描画座標に合わせてクリックヒット判定し、選択した `charId` を親へ送る。
- `SboSvr/webroot/app.js`
  - `handleAdminGameMessage()` が iframe からの `postMessage` を受信。
  - `openCharacterDetailFromGame()` が `char-detail-id-input` を更新し、`fetchCharacterDetail()` を呼ぶ。
- `SboSvr/webroot/index.html`
  - 左側にゲーム iframe を常設し、右側をツール / 編集パネルとして切り替える。

この経路は「同一ブラウザ内のゲーム画面と管理画面をつなぐ」用途に限定する。複数管理者間の同期やネイティブ管理者との同期が必要な選択は、従来の selection API / `/ws/admin` を使う。
...
つまりサーバー管理の選択は「Web ↔ Web」「ネイティブ → Web」の 2 方向。現状 **Web → ネイティブ** の逆方向は実装されていない（ゲーム画面側に Web の選択を反映する要件がない）。
...
- **直接 `postMessage` 連携はキャラクター選択のみ** 実装済み。マップセル、配置物、イベント選択は次フェーズ。
