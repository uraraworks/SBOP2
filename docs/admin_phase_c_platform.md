...
2. **API ハンドラ登録レイヤーの実装**
   - `IApiHandler` インターフェースを定義し、HTTP メソッド・パス・認証ロールを宣言的に記述できるようにする。
   - ルーティングテーブル生成を `ApiRouterBuilder` で自動化し、起動時にハンドラを登録。
...
| ルーティング | `IApiHandler` と `ApiRouterBuilder` 実装 | 共通ライブラリコード、サンプルハンドラ | サーバー基盤担当 | サンプルハンドラで CRUD テスト、エラーハンドリング確認 |
